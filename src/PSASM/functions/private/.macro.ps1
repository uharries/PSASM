...
"@
	...
	$FunctionText = if ([string]::IsNullOrWhiteSpace($ParamList)) {
		...
		# "[PSASM(macro)] param([object[]]`$macroArgs)`n`$macroArgs.GetType() | ft -auto | out-string | write-host`n`$macroArgs |  out-string | write-host`n$UnpackCode`n$BodyText"
		"[PSASM(macro)] param([object[]]`$macroArgs)`n$UnpackCode`n$BodyText"
	}

	$sb = [scriptblock]::Create($FunctionText)
	# set-item "Function:\script:$Name" -Value $sb

	$psasm.Macros[$ScopeID] ?? ($psasm.Macros[$ScopeID] = [ordered]@{})

	$psasm.Macros[$ScopeID][$Name] = $sb
